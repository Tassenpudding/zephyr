...
#ifdef CONFIG_ADC_MCUX_LPC_ADC_STREAM
	/*
	 * The request one completed block is handed to. The DMA callback takes
	 * it, so a request is completed exactly once; the capture runs on
	 * between submissions.
	 */
	atomic_ptr_t sqe;
...
static int mcux_lpc_adc_check_buffer_size(const struct adc_sequence *sequence)
{
	...
	if (sequence->buffer_size < needed) {
		LOG_ERR("Sequence buffer of %zu bytes is smaller than the %zu bytes needed",
...
static int mcux_lpc_adc_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	ret = mcux_lpc_adc_check_buffer_size(sequence);
	if (ret != 0) {
		return ret;
	}
				...
				int status)
{
	...
	/* The DMA has moved on to the other buffer regardless of what follows. */
	src = data->stream_bufs[data->stream_buf_idx];
	data->stream_buf_idx ^= 1U;

	/*
	 * Take the pending request so it is completed once. The capture is free
	 * running, so a block that lands before the consumer submits again has
	 * nowhere to go and is dropped; the consumer sees a gap.
	 */
	iodev_sqe = atomic_ptr_set(&data->sqe, NULL);
	if (iodev_sqe == NULL) {
		return;
	}
	...
	if (rtio_sqe_rx_buf(iodev_sqe, need, need, &buf, &buf_len) != 0) {
...
static void mcux_lpc_adc_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	/*
	 * One completion per submission, against a capture that keeps running:
	 * a resubmission only hands over the new request, the hardware is set
	 * up by the first one.
	 */
	if (data->stream_started) {
		atomic_ptr_set(&data->sqe, iodev_sqe);
		return;
	}
	...
	/* Nothing triggers until the timer runs, so arm the request last. */
	atomic_ptr_set(&data->sqe, iodev_sqe);
