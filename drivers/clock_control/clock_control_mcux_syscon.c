...
#if defined(CONFIG_DISPLAY_MCUX_LCDC)
	if ((uint32_t)sub_system == MCUX_LCD_CLK) {
		CLOCK_EnableClock(kCLOCK_Lcd);
	}
#endif

#if defined(CONFIG_MEMC_MCUX_EMC)
	if ((uint32_t)sub_system == MCUX_EMC_CLK) {
